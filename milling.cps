/**
  ...
  $Revision: 43012 7e57a1fe748d1d22308c743d193446a5137d6b21 $
  $Date: 2020-11-23 09:05:38 $
  
  FORKID {3CFDE807-BE2F-4A4C-B12A-03080F4B1285}
*/

description = "Tormach PathPilot - restored rapids";
vendor = "Tormach";
...
properties = {
  ...
  disableCoolant: true, // disables all coolant codes
  ...
  minRapidHeight: 2 // minimum height above stock top for rapid moves
};
...
propertyDefinitions = {
  ...
  minRapidHeight: {title:"Minimum rapid move height", description:"The minimum Z height above the stock top for rapid moves.", type:"number"}
};
...
function writeMovementType() {
/*
  writeBlock("(onSection movement = ", movement, ")");
  if (movement == MOVEMENT_LEAD_IN) {
    writeBlock("(MOVEMENT_LEAD_IN)");
  } else if (movement == MOVEMENT_LEAD_OUT) {
    writeBlock("(MOVEMENT_LEAD_OUT)");
  } else if (movement == MOVEMENT_PLUNGE) {
    writeBlock("MOVEMENT_PLUNGE)");
  } else if (movement == MOVEMENT_RAMP_PROFILE) {
    writeBlock("MOVEMENT_RAMP_PROFILE)");
  } else if (movement == MOVEMENT_RAMP) {
    writeBlock("MOVEMENT_RAMP)");
  } else if (movement == MOVEMENT_RAMP_HELIX) {
    writeBlock("MOVEMENT_RAMP_HELIX)");
  } else if (movement == MOVEMENT_RAMP_PROFILE) {
    writeBlock("(MOVEMENT_RAMP_PROFILE)");
  } else if (movement == MOVEMENT_RAMP_ZIG_ZAG) {
    writeBlock("(MOVEMENT_RAMP_ZIGZAG)");
  } else if (movement == MOVEMENT_LINK_DIRECT) {
    writeBlock("(MOVEMENT_LINK_DIRECT)");
  } else if (movement == MOVEMENT_LINK_TRANSITION) {
    writeBlock("(MOVEMENT_LINK_TRANSITION)");
  } else if (movement == MOVEMENT_CUTTING) {
    writeBlock("(MOVEMENT_CUTTING)");
  } else if (movement == MOVEMENT_EXTENDED) {
    writeBlock("(MOVEMENT_EXTENDED)");
  } else if (movement == MOVEMENT_HIGH_FEED) {
    writeBlock("(MOVEMENT_HIGH_FEED)");
  } else if (movement == MOVEMENT_RAPID) {
    writeBlock("(MOVEMENT_RAPID)");
  } else if (movement == MOVEMENT_FINISH_CUTTING) {
    writeBlock("(MOVEMENT_FINISH_CUTTING)");
  } else if (movement == MOVEMENT_REDUCED) {
    writeBlock("(MOVEMENT_REDUCED)");
  }
*/  
}
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    //writeBlock("(movement = ", movement, ")")
    writeBlock(gMotionModal.format(0), x, y, z, "(Actual rapid)");
    feedOutput.reset();
  }
}

// When rapidMode is true any X/Y only move can be converted to a rapid
// move on the assumption the tool is above the stock and fixtures.
// A Z-only move to >= minRapidHeight will be converted to a rapid move and
// set rapidMode to true.
// Any move to Z < minRapidHeight will set rapidMode to false.
var rapidMode = false;
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (! (x || y)) {
    if (_z >= properties.minRapidHeight) {
      writeBlock(gMotionModal.format(0), x, y, z, "(Assumed rapid, rapidMode now true)");
      feedOutput.reset();
      rapidMode = true;
      return;
    }
  ...
  if (rapidMode && ! z) {
    // Any X/Y only move can be converted to a rapid
    writeBlock(gMotionModal.format(0), x, y, z, "(In rapidMode, X/Y only move converted)");
    feedOutput.reset();
    return;
  }

  if (rapidMode && z && _z < properties.minRapidHeight) {
    writeComment("Z move below minRapidHeight, rapidMode now false");    
    rapidMode = false;
  }
